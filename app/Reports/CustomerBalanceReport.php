...
{
    public function generate(array $filters = [], int $perPage = null): \Illuminate\Pagination\LengthAwarePaginator
    {
        ...
        $customers = $query->paginate($perPage ?? 10);

        // Transform the paginated data
        $transformedData = $customers->getCollection()->map(function ($customer) use ($startDate, $endDate) {
        ...
        // Return paginated results with transformed data
        return $customers->setCollection($transformedData);
    }
    ...
    public function getSummary(array $filters = []): array
    {
        ...
        // Get all customers for summary (not paginated)
        $customers = $query->get();

        $data = $customers->map(function ($customer) use ($startDate, $endDate) {
            // Calculate total ATC value (transactions within date range)
            $totalAtcValue = $customer->transactions()
                ->whereBetween('date', [$startDate, $endDate])
                ->get()
    ...
    public function getChartData(array $filters = []): array
    {
        ...
        // Get all customers for chart data (not paginated)
        $customers = $query->get();
